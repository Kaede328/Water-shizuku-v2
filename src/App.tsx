...
import { Plus, RotateCcw, Undo, BarChart2, Bell, Sparkles, Moon, Sun } from 'lucide-react';
...
export default function App() {
  ...
  const [recordTimes, setRecordTimes] = useState<number[]>([]); // 記録時間のスタンプ
  ...
  // 1時間以内に1000ml以上飲んだかチェック
  const checkOverhydration = (newAmount: number) => {
    const now = Date.now();
    const oneHourAgo = now - 3600000;
    const recentTotal = recordTimes.filter(t => t > oneHourAgo).length * 250 + newAmount; // 簡易計算
    if (recentTotal >= 1000) {
      alert("楓さん、少しずつ飲むのが体に優しいですよ。ゆっくり休みながら飲みましょうね。");
    }
  };
  ...
  const triggerHaptic = (type: 'light' | 'medium' | 'success') => {
    if (window.navigator?.vibrate) {
      ...
      if (type === 'success') window.navigator.vibrate([10, 30, 10, 30]);
    }
  };
  ...
  const addWater = (amount: number) => {
    ...
    if ((totalToday < 1000 && newTotal >= 1000) || (totalToday < 2000 && newTotal >= 2000) || (totalToday < 2500 && newTotal >= 2500)) {
  ...
  return (
    <div className={`h-screen w-full transition-colors duration-1000 font-sans overflow-hidden flex flex-col items-center justify-between py-4 px-6 relative ${
      isDarkMode 
      ? 'bg-gradient-to-b from-slate-900 via-indigo-950 to-slate-900 text-sky-100' 
      : 'bg-gradient-to-b from-white via-sky-50 to-sky-200 text-sky-900'
    }`}>
      
      <header className="w-full flex justify-between items-center pt-2 px-2 z-50">
        <button className="p-2 opacity-40">{isDarkMode ? <Moon className="w-4 h-4" /> : <Sun className="w-4 h-4" />}</button>
      ...
      <div className="flex flex-col items-center gap-4">
        ...
        <div className="relative w-52 h-52 flex-shrink-0">
          <AnimatePresence>
            {showCelebrate && (
              <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.2 }} className="absolute inset-0 rounded-full overflow-hidden z-0">
                <div className={`absolute inset-0 blur-2xl ${isDarkMode ? 'bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500/50' : 'bg-gradient-to-r from-teal-200/50 via-sky-300/60 to-indigo-200/50'}`} />
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <div className={`absolute inset-0 rounded-full overflow-hidden z-10 ${isDarkMode ? 'bg-indigo-900/20' : 'bg-sky-50/20'}`}>
            <motion.div className="absolute bottom-[-15%] left-[-50%] right-[-50%]" style={{ background: isDarkMode ? 'radial-gradient(circle, #4338ca 0%, #1e1b4b 100%)' : 'radial-gradient(circle, #38bdf866 0%, #0ea5e980 100%)' }} animate={{ height: `${waterPercentage + 11}%`, borderRadius: ["38% 42% 40% 40%", "45% 35% 42% 38%", "35% 45% 35% 45%", "38% 42% 40% 40%"], rotate: [0, 5, -3, 0] }} transition={{ height: { duration: 1.5, ease: [0.4, 0, 0.2, 1] }, borderRadius: { duration: 13, repeat: Infinity, ease: "easeInOut" }, rotate: { duration: 17, repeat: Infinity, ease: "easeInOut" } }} />
            <motion.div className="absolute bottom-[-15%] left-[-50%] right-[-50%]" style={{ background: isDarkMode ? 'linear-gradient(180deg, #6366f144 0%, #4338ca22 100%)' : 'linear-gradient(180deg, #bae6fd4d 0%, #38bdf833 100%)' }} animate={{ height: `${waterPercentage + 13}%`, borderRadius: ["42% 38% 44% 36%", "38% 42% 35% 45%", "44% 36% 40% 40%", "42% 38% 44% 36%"], rotate: [0, -6, 4, 0] }} transition={{ height: { duration: 1.5, ease: [0.4, 0, 0.2, 1] }, borderRadius: { duration: 8, repeat: Infinity, ease: "easeInOut" }, rotate: { duration: 11, repeat: Infinity, ease: "easeInOut" } }} />
          </div>
            ...
            <AnimatePresence>
              {showCelebrate ? (
                ...
                <div className="flex flex-col items-center">
                  <span className={`text-2xl font-extralight ${isDarkMode ? 'text-sky-100' : 'text-sky-900'}`}>{totalToday % 1000}</span>
                  <span className={`text-[8px] uppercase font-bold tracking-widest ${isDarkMode ? 'text-indigo-400' : 'text-sky-500/60'}`}>ml / 1000</span>
                </div>
              )}
            </AnimatePresence>
          </div>
                  ...
                  <BarChart data={chartData}>
                    ...
                    <Bar dataKey="amount" radius={[4, 4, 0, 0]}>
                      {chartData.map((entry, index) => (
                        <Cell key={`cell-${index}`} fill={entry.amount >= 2000 ? (isDarkMode ? '#818cf8' : '#38bdf8') : (isDarkMode ? '#312e81' : '#bae6fd')} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
      ...
      <AnimatePresence>
        {showConfirm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-50 bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-6">
